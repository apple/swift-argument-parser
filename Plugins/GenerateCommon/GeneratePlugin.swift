...
protocol GeneratePlugin: CommandPlugin {
  ...
  func performCommand(
    ...
    if extractor.helpRequest() {
      ...
      print(
        ...
        NOTE: The "\(Self.pluginName)" plugin handles passing the "<tool>" and
    ...
    guard buildResult.succeeded else {
      throw GeneratePluginError.buildFailed(buildResult.logText)
    }
    ...
    for builtArtifact in buildResult.builtArtifacts {
      ...
      print("Generated \(Self.artifactName) in '\(outputDirectory)'")
    }
  }
}
