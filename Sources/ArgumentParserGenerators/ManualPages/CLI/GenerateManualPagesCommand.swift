...
struct GenerateManualPagesCommand: ParsableCommand {
  ...
  static let configuration = CommandConfiguration(
    commandName: "manual-pages",
    abstract: "Generate manual pages.")

  @Argument(help: "Tool to generate manual pages for.")
  var tool: String

  @Flag(help: "Generate a single page with information for all subcommands.")
  var singlePage = false

  @Option(name: .long, help: "Override the creation date of the manual page.")
  var date: Date = Date()

  @Option(name: .long, help: "Section of the manual.")
  var section: Int = 1

  @Option(name: .long, help: "Names and/or emails of the tool's authors. Supplied as 'name<email>'.")
  var authors: [AuthorArgument] = []

  @Option(name: .shortAndLong, help: "Directory to save generated man-pages.")
  ...
  func validate() throws {
    ...
    // outputDirectory must already exist, `GenerateManPage` will not create it.
  ...
  func run() throws {
    ...
    do {
      ...
      let output = try executeCommand(executable: tool, arguments: ["--dump-help"])
