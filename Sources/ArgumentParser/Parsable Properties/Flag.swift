...
/// command-line flag. A *flag* is a dash-prefixed label that can be provided
/// on the command line, such as `-d` and `--debug`.
///
/// For example, the following program declares a flag that lets a user
/// indicate that seconds should be included when printing the time.
///
/// ```swift
/// @main
...
extension Flag where Value == Bool {
  ...
  /// Creates a Boolean property with default value provided by standard Swift
  /// default value syntax that reads its value from the presence of one or
  /// more inverted flags.
  ///
  ...
  /// Creates a Boolean property with no default value that reads its value
  /// from the presence of one or more inverted flags.
  ///
  /// Use this initializer to create a Boolean flag with an on/off pair. With
  /// the following declaration, for example, the user can specify either
  /// `--use-https` or `--no-use-https` to set the `useHTTPS` flag to `true` or
...
extension Flag where Value: EnumerableFlag {
  ...
  /// default value syntax that gets its value from the presence of a flag.
  ///
  /// Use this initializer to customize the name and number of states further
  /// than using a `Bool`. To use, define an `EnumerableFlag` enumeration with
  /// a case for each state, and use that as the type for your flag. In this
  /// case, the user can specify either `--use-production-server` or
...
extension Flag {
  ...
  /// the presence of zero or more flags, where the allowed flags are defined
  /// by an `EnumerableFlag` type.
  ///
